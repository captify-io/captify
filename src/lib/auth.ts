...
import { storeTokensSecurely, getStoredTokens, removeStoredTokens } from "@captify-io/base/lib/auth-store";
...
// Only check NEXTAUTH_URL since that's the actual URL being used
const isProductionDomain = process.env.NEXTAUTH_URL?.includes('captify.io');
  ...
  cookies: {
    sessionToken: {
      ...
      options: {
        ...
        secure: isProduction,
        ...(isProduction && { domain: ".captify.io" }),
...
// Export the config and refresh function for external apps to use
export { authConfig, refreshAccessToken };
